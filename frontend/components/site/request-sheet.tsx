...
export function RequestSheet({ open, onOpenChange, wallet, wrongNetwork, progress, onSubmit }: { open: boolean; onOpenChange: (open: boolean) => void; wallet: WalletConnection | null; wrongNetwork: boolean; progress: WriteProgress | null; onSubmit: (args: (string | boolean)[]) => Promise<void> }) {
  ...
  if (!open) return null;
  return <div className="site-dialog-backdrop" role="presentation" onMouseDown={() => onOpenChange(false)}><div className="site-dialog-content" role="dialog" aria-modal="true" aria-labelledby="request-sheet-title" onMouseDown={(event) => event.stopPropagation()}><button type="button" className="site-dialog-close" aria-label="Close request form" onClick={() => onOpenChange(false)}><X className="size-5" /></button><div><p className="site-kicker">/NEW AUTHORIZATION</p><h2 id="request-sheet-title">Request access.</h2><p className="site-dialog-description">Bind a new use to the live policy. Simulation runs before any wallet prompt.</p></div><form onSubmit={submit} className="site-request-form"><div className="site-form-note"><span>POLICY</span><strong>{studioDevConfig.policyId}</strong><span className="site-form-wallet">{wallet ? wrongNetwork ? "Wrong network" : "Wallet ready" : "Wallet required"}</span></div><div className="site-form-grid"><Field label="Request ID" value={values.requestId} placeholder="request-2026-..." onChange={(value) => update("requestId", value)} /><Field label="Resource ID" value={values.resourceId} placeholder="profile-..." onChange={(value) => update("resourceId", value)} /><Field label="Replay nonce" value={values.replayNonce} placeholder="unique nonce" onChange={(value) => update("replayNonce", value)} /><Field label="Purpose" value={values.purpose} placeholder="account access verification" onChange={(value) => update("purpose", value)} /><Field label="Data category" value={values.dataCategory} placeholder="identity profile" onChange={(value) => update("dataCategory", value)} /><Field label="Recipient" value={values.recipient} placeholder="ConsentGate Demo App" onChange={(value) => update("recipient", value)} /><Field label="Request expiry UTC" value={values.requestExpiresAt} placeholder="2026-09-20T15:22:29Z" onChange={(value) => update("requestExpiresAt", value)} /><Field label="Retention until UTC" value={values.retentionUntil} placeholder="2026-09-20T03:22:29Z" onChange={(value) => update("retentionUntil", value)} /><label className="site-checkbox"><input type="checkbox" checked={values.commercialUse} onChange={(event) => update("commercialUse", event.target.checked)} /> Commercial use</label></div>{error ? <p className="site-form-error">{error}</p> : null}<div className="site-dialog-footer"><button type="button" className="editorial-button editorial-button-outline" onClick={() => onOpenChange(false)}>Cancel</button><button type="submit" disabled={!wallet || wrongNetwork || submitting || progress?.phase === "finalizing"} className="editorial-button editorial-button-dark"><Send className="size-4" />{submitting ? "Preparing…" : "Simulate & create request"}<ArrowUpRight className="size-4" /></button></div></form></div></div>;
}
